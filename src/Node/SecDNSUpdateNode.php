...
class SecDNSUpdateNode
{
    public static function create(RequestInterface $request, \DOMElement $parentNode, bool $isUrgent = false): \DOMElement
    {
        ...
        $node = $request->getDocument()->createElement('secDNS:update');
        if ($isUrgent) {
            $node->setAttribute('urgent', 'true');
        }
